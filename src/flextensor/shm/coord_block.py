...
# 4-byte protocol version = 72 bytes total
_VERSION_MAX_LEN = 64
COORD_HEADER_SIZE = 4 + _VERSION_MAX_LEN + 4  # 72 bytes
_COORD_HEADER_FORMAT = f"!I{_VERSION_MAX_LEN}sI"
...
class CoordBlockHeader:
    ...
    def write_to(self, buf: bytearray | memoryview, offset: int = 0) -> None:
        ...
        struct.pack_into(_COORD_HEADER_FORMAT, buf, offset, len(version_bytes), padded, self.protocol_version)
    ...
    def read_from(cls, buf: bytes | bytearray | memoryview, offset: int = 0) -> CoordBlockHeader:
        ...
        """
        version_len, version_bytes, protocol = struct.unpack_from(_COORD_HEADER_FORMAT, buf, offset)
        version_str = version_bytes[:version_len].decode("utf-8")
        return cls(flextensor_version=version_str, protocol_version=protocol)
