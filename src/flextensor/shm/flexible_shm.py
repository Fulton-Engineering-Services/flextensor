...
class KeepAlive:
    ...
    def __init__(
        ...
        try:
            ...
            try:
                ...
                self.lock.release()
        except (posix_ipc.BusyError, RuntimeError, BlockingIOError) as e:
    ...
    def stop(self) -> None:
        ...
        with self.lock:
            # Only pop if the process_id still exists (handles multiple instances per process)
            if self.process_id in self.keep_alive_dict:
        ...
        with self.lock:
            ...
            for pid, timestamp in alive_items:
                if timestamp < current_time - self.keep_alive_seconds:
                    ...
                    removed_count += 1
            return len(alive_items) - removed_count > 0
...
class FlexibleSharedMemory:
    ...
    def __init__(
        ...
        self.main_lock = self.lock_class(lock_name, locked=True, non_blocking=False)
