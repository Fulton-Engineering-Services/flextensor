...
*[inference state]: Third active state — applies the computed offloading strategy for production execution. No timing collection.
...
*[knapsack_scale]: Scale factor for the knapsack algorithm that controls the trade-off between offloading aggressiveness and overhead.
...
*[profiling state]: Second active state — measures per-trap execution timing using CUDA events across profile_iters iterations. "Profiling" is the process; the offload profile is the artifact.
...
*[warmup state]: First active state — discovers which parameters belong to which traps by intercepting PyTorch operations.
